...
if __name__ == "__main__":
    ...
    sonuc = metro.en_hizli_rota_bul("M4", "K1")
    if sonuc:
        rota, sure = sonuc
        print(f"En hızlı rota ({sure} dakika):", " -> ".join(i.ad for i in rota))

    # Senaryo 6: Demetevler'den AŞTİ'ye (Aktarma Testi)
    print("\n6. Demetevler'den AŞTİ'ye:")
    rota = metro.en_az_aktarma_bul("K3", "M1")
    if rota:
        print("En az aktarmalı rota:", " -> ".join(i.ad for i in rota))

    sonuc = metro.en_hizli_rota_bul("K3", "M1")
    if sonuc:
        rota, sure = sonuc
        print(f"En hızlı rota ({sure} dakika):", " -> ".join(i.ad for i in rota))
